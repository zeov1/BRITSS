...
def binary_cross_entropy_with_logits(inpt, target, weight=None, size_average=True, reduce=True):
    if not (target.size() == inpt.size()):
        raise ValueError(f"Target size ({target.size()}) must be the same as input size ({inpt.size()})")

    max_val = (-inpt).clamp(min=0)
    loss = inpt - inpt * target + max_val + ((-max_val).exp() + (-inpt - max_val).exp()).log()
